            ...
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="card-title mg-b-0">{{ trans('category.categories') }}</h4>
                        <div class="d-flex">
                            <a class="btn btn-primary text-white" href="{{ route('admin.categories.create') }}">
                                {{ trans('category.add_new_category') }}
                            </a>
                            &nbsp;&nbsp;
                            <a class="btn btn-primary text-white" href="{{ route('admin.categories.trash') }}">
                                {{ trans('category.trashed_categories') }}
                            </a>
                        </div>
                    </div>
                </div>
                        ...
                        <table id="example" class="table key-buttons text-md-nowrap">
                            ...
                            <tbody>
                                @foreach ($categories as $category)
                                    <tr>
                                        <td><img width="50" height="50" src="{{ $category->img_path }}"
                                                alt="" srcset=""></td>
                                        ...
                                        @canany(['view_category', 'edit_category', 'delete_category', 'active_category',
                                            ...
                                            <td class="row pl-3">
                                                @can('view_category')
                                                    <a href="{{ route('admin.subcategories.index', $category->slug) }}"
                                                        class="btn btn-info" data-placement="top" data-toggle="tooltip"
                                                        data-original-title="{{ trans('category.show_subcategories') }}"><i
                                                ...
                                                @can('active_category')
                                                    @if ($category->active == 1)
                                                        <a href="{{ route('admin.categories.active', $category->slug) }}"
                                                ...
                                                @can('add_home_category')
                                                    @if ($category->appear_home == 1)
                                                        <a href="{{ route('admin.categories.appear_home', $category->slug) }}"
                                                            class="btn btn-danger" data-placement="top" data-toggle="tooltip"
                                                ...
                                                @can('delete_category')
                                                    <form id="delete-form-{{ $category->slug }}"
                                                        ...
                                                        @csrf
                                                        @method('DELETE')
                                                        <button class="btn btn-danger delete-btn" data-id="{{ $category->slug }}"
